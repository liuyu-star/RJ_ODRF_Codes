...
for (i.data in dataA)
{
  ...
  # A <- try(rotationForest(data.frame(X), y, L = 1), silent = TRUE)
  ...
  t0 <- proc.time()
  A <- HHDecisionTree(
    response = "classify", n_trees = 1, n_folds = 1,
    pruning = FALSE, min_node_impurity = 0.0
  ) #' regressor'
  HHCART <- A$fit(X, y)
  # HHCART <- A$fit(y ~ ., data = data.frame(X, y = y))
  pred <- predict(HHCART, newdata = data.frame(X1))
  e.ODT <- mean(pred != y1)
  t1 <- proc.time()
  t.ODT <- c(t1 - t0)[3]
  c.ODT <- length(which(hhcartr::get_tree_structure(HHCART)$node_type == "leaf"))
  # bagging_predict()
